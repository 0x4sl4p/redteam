...
ascii_banner = pyfiglet.figlet_format("R3DT34M SOFTWALL T00LS ", font = "digital")
print(ascii_banner)
ascii_banner2 = pyfiglet.figlet_format("First-Infra \n sl4p", font = "banner")

PROG_VERS='1.0'
PROG_DATE='29/04/2022'


broadcast = subprocess.check_output("ifconfig | grep broadcast | cut -d ' ' -f 16", universal_newlines=True, shell=True).strip()

comandos=[
	[['ifconfig'],3, "IP Recebido automáticamente"],
	[['ping', '-c6', '-b', broadcast],3, "Teste Smurf Attack"],
...
comandosComTempo=[
	...
	[['nc', '-v', 'ftp.unicamp.br', '21'],5,"Acesso a servidor FTP externo"],
...
def main():
	try:	
		...
		if args.quiet:
			...
			print(colored(ascii_banner2, 'yellow'))
			time.sleep(4)
